...
\begin{center}
  \begin{tikzpicture}
    \draw[gray,ultra thin] (-.9,-.9) grid (4.9,4.9);
    \draw[->] (-1.1,0) -- (5.1,0) node [pos=1,above] {$X$};
    \draw[->] (0,-1.1) -- (0,5.1) node [pos=1,right] {$Y$};

    \begin{scope}
      \clip (-1,-1) rectangle (6,5);
      \draw[variable=\x,domain=0:4] plot (\x, 0.75*1.5^\x) node[right] {$1 < g$};
      \draw[variable=\x,domain=0:4] plot (\x, 1.5*1^\x) node[right] {$g = 1$};
      \draw[variable=\x,domain=0:4] plot (\x, 4*0.65^\x) node[right] {$0 < g < 1$};
    \end{scope}
  \end{tikzpicture}
\end{center}
Algemene vorm:
...
\begin{itemize}
  ...
  \item Indien $g=1$ ontaardt $f$ in een constante functie
\end{itemize}


\input{oefeningen/oefgroei}
