...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \seqlink<7->[label=rad]{init}{turn wheel function}
      \seqlink<7->{turn wheel function}{exit}
      ...
      \seqpoint<7->{turn wheel function}{draai het rad}
      \seqpoint[placement=below]{vowel}{gok klinker}
      \seqpoint[placement=below]{buy vowel}{score -= 50}
      \seqpoint[placement=below]{show vowels}{toon}
      ...
      \only<7>{
        \node[anchor=north] at ($ (bankrupt function init) ! .5 ! (bankrupt function exit) + (0,-.5) $) {\tiny draai het raad};
      }
    \end{tikzpicture}
  \end{center}
\end{frame}
